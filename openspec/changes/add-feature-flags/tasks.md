...
- [x] 5.2 Verify propagation across output formats — tags ride the shared RunTags machinery all outputs consume; verified end-to-end on the JSON output
...
- [x] 6.4 Rewire `K6_PROMETHEUS_RW_TREND_AS_NATIVE_HISTOGRAM` as a Phase-1 honored alias for `native-histograms` (output now consumes the flag)
...
- [x] 8.3 Test version-skew degradation: unknown name on worker logs `ERROR`, run continues, feature not applied (same non-fatal Unknown path)
